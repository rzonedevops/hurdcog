...
function makeMenu(spec) {
    ...
    const toc = Toc_1.default.build(spec, { expandy: true });
    ...
    const json = JSON.stringify({ refsByClause: spec.refsByClause, entries: spec.biblio.localEntries() }, biblioReplacer);
    return {
...
function biblioReplacer(k, v) {
    if (!{}.hasOwnProperty.call(this, 'type')) {
        // for non-BiblioEntry items
        return v;
    }
    if (k === 'referencingIds' && v.length === 0) {
        return undefined;
    }

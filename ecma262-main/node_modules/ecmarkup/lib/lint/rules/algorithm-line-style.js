...
- `Other:` + substeps
*/
function default_1(report, node, algorithmSource, parsedSteps) {
    const stepSeq = parsedSteps.get(node);
    if (stepSeq == null || stepSeq.items.length === 0) {
        return;
    }
    function locate(offset) {
        return (0, utils_1.offsetToLineAndColumn)(algorithmSource, offset);
    }
    ...
    if (/^(?:If |Else if)/.test(initialText)) {
        if (hasSubsteps) {
            ...
            const lineSource = algorithmSource.slice(first.location.start.offset, last.location.end.offset);
